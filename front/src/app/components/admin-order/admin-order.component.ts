...
export class AdminOrderComponent implements OnInit {
  ...
  public orderStatues = ['待确认', '待付款', '待核实', "待发货", "已发货", "已完成", '已取消'];
  public filterOrderStatue = -1
  ...
  ngOnInit() {
    ...
    this._getData().subscribe((data) => this.filtedData = this.data = data);
  ...
  public filterOrderStatueChange() {
    console.log(this.data)
    setTimeout(() => {
      if (this.filterOrderStatue == -1) {
        this.filtedData = this.data
      } else {
        this.filtedData = this.data.filter(i => i.status == this.filterOrderStatue)
      }
    })
